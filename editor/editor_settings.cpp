...
void EditorSettings::_load_defaults(Ref<ConfigFile> p_extra_config) {
...

	{
		...
		// Some locales are not properly supported currently in Active due to lack of font shaping
	...
	hints["interface/theme/preset"] = PropertyInfo(Variant::STRING, "interface/theme/preset", PROPERTY_HINT_ENUM, "Default,Alien,Arc,Active 2,Grey,Light,Solarized (Dark),Solarized (Light),Custom", PROPERTY_USAGE_DEFAULT);
	...
	_initial_set("interface/theme/base_color", Color(0.27, 0.27, 0.27));
	...
	_initial_set("interface/theme/accent_color", Color(1.0, 0.6, 0.3));
	...
	_initial_set("interface/theme/contrast", 0.15);
	...
	hints["editors/3d/navigation/navigation_scheme"] = PropertyInfo(Variant::INT, "editors/3d/navigation/navigation_scheme", PROPERTY_HINT_ENUM, "Active,Maya,Modo");
...
#endif
	// Should match the ANDROID_WINDOW_* constants in 'platform/android/java/editor/src/main/java/org/activeengine/editor/ActiveEditor.kt'
	...
	// TRANSLATORS: Project Manager here refers to the tool used to create/manage Active projects.
...
void EditorSettings::create() {
	...
	if (self_contained) {
		...
		data_dir = data_path.plus_file(OS::get_singleton()->get_active_dir_name());
		// Can be different from data_path e.g. on Linux or macOS
		config_path = OS::get_singleton()->get_config_path();
		config_dir = config_path.plus_file(OS::get_singleton()->get_active_dir_name());
		// Can be different from above paths, otherwise a subfolder of data_dir
		cache_path = OS::get_singleton()->get_cache_path();
		if (cache_path == data_path) {
			cache_dir = data_dir.plus_file("cache");
		} else {
			cache_dir = cache_path.plus_file(OS::get_singleton()->get_active_dir_name());
		}
	}
