...
private:
	...
	String _test_path() {
		...
		if (mode == MODE_IMPORT || mode == MODE_RENAME) {
			if (valid_path != "" && !d->file_exists("project.active")) {
				if (valid_path.ends_with(".zip")) {
					...
					while (ret == UNZ_OK) {
						...
						if (String::utf8(fname).ends_with("project.active")) {
							break;
						}
					...
					if (ret == UNZ_END_OF_LIST_OF_FILE) {
						set_message(TTR("Invalid \".zip\" project file; it doesn't contain a \"project.active\" file."), MESSAGE_ERROR);
						memdelete(d);
						get_ok()->set_disabled(true);
						unzClose(pkg);
						return "";
					...
					set_message(TTR("Please choose a \"project.active\" or \".zip\" file."), MESSAGE_ERROR);
					memdelete(d);
				...
				set_message(TTR("This directory already contains a Active project."), MESSAGE_ERROR, INSTALL_PATH);
				memdelete(d);
				get_ok()->set_disabled(true);
				return "";
			}
	...
	void _browse_path() {
		...
		if (mode == MODE_IMPORT) {
			...
			fdialog->add_filter(vformat("project.active ; %s %s", VERSION_NAME, TTR("Project")));
	...
	void ok_pressed() {
		...
		if (mode == MODE_RENAME) {
			...
			// Load project.active as ConfigFile to set the new name.
			ConfigFile cfg;
			String project_active = dir2.plus_file("project.active");
			Error err = cfg.load(project_active);
			if (err != OK) {
				set_message(vformat(TTR("Couldn't load project at '%s' (error %d). It may be missing or corrupted."), project_active, err), MESSAGE_ERROR);
			} else {
				cfg.set_value("application", "config/name", project_name->get_text().strip_edges());
				err = cfg.save(project_active);
				if (err != OK) {
					set_message(vformat(TTR("Couldn't save project at '%s' (error %d)."), project_active, err), MESSAGE_ERROR);
				}
			}
			...
			if (mode == MODE_IMPORT) {
				...
				if (mode == MODE_NEW) {
					...
					if (ProjectSettings::get_singleton()->save_custom(dir.plus_file("project.active"), initial_settings, Vector<String>(), false) != OK) {
						set_message(TTR("Couldn't create project.active in project path."), MESSAGE_ERROR);
					} else {
					...
					while (ret == UNZ_OK) {
						...
						if (name.ends_with("project.active")) {
							zip_root = name.substr(0, name.rfind("project.active"));
							break;
						}
...
public:
	...
	void show_dialog() {
		if (mode == MODE_RENAME) {
			...
			// Fetch current name from project.active to prefill the text input.
			ConfigFile cfg;
			String project_active = project_path->get_text().plus_file("project.active");
			Error err = cfg.load(project_active);
			if (err != OK) {
				set_message(vformat(TTR("Couldn't load project at '%s' (error %d). It may be missing or corrupted."), project_active, err), MESSAGE_ERROR);
				status_rect->show();
...
void ProjectList::load_project_data(const String &p_property_key, Item &p_item, bool p_favorite) {
	...
	String conf = path.plus_file("project.active");
	bool grayed = false;
	bool missing = false;
	...
	if (config_version > ProjectSettings::CONFIG_VERSION) {
		// Comes from an incompatible (more recent) Active version, grey it out
		grayed = true;
	}
...
void ProjectList::load_projects() {
	...
	for (List<PropertyInfo>::Element *E = properties.front(); E; E = E->next()) {
		// This is actually something like "projects/C:::Documents::Active::Projects::MyGame"
		String property_key = E->get().name;
...
void ProjectList::update_dock_menu() {
	...
	for (int i = 0; i < _projects.size(); ++i) {
		if (!_projects[i].grayed && !_projects[i].missing) {
			...
			OS::get_singleton()->global_menu_add_item("_dock", _projects[i].project_name + " ( " + _projects[i].path + " )", GLOBAL_OPEN_PROJECT, Variant(_projects[i].path.plus_file("project.active")));
			total_added++;
		}
	}
	if (total_added != 0) {
...
void ProjectManager::_open_selected_projects_ask() {
	...
	String conf = project.path.plus_file("project.active");
	int config_version = project.version;

	// Check if the config_version property was empty or 0
	if (config_version == 0) {
		ask_update_settings->set_text(vformat(TTR("The following project settings file does not specify the version of Active through which it was created.\n\n%s\n\nIf you proceed with opening it, it will be converted to Active's current configuration file format.\nWarning: You won't be able to open the project with previous versions of the engine anymore."), conf));
	...
	if (folders_set.size() > 0) {
		...
		if (folders.size() == 1) {
			...
			if (dir->change_dir(folders[0]) == OK) {
				...
				while (confirm && file != String()) {
					if (!dir->current_is_dir() && file.ends_with("project.active")) {
						confirm = false;
					}
					file = dir->get_next();
				}
		...
		if (confirm) {
			...
			multi_scan_ask->set_text(
					vformat(TTR("Are you sure to scan %s folders for existing Active projects?\nThis could take a while."), folders.size()));
...
ProjectManager::ProjectManager() {
	...
	String cp;
	cp += 0xA9;
	// TRANSLATORS: This refers to the application where users manage their Active projects.
