...
void EditorNode::_notification(int p_what) {
	switch (p_what) {
		...
		case EditorSettings::NOTIFICATION_EDITOR_SETTINGS_CHANGED: {
			...
			p->set_item_icon(p->get_item_index(HELP_ABOUT), gui_base->get_icon("Active", "EditorIcons"));
			p->set_item_icon(p->get_item_index(HELP_SUPPORT_ACTIVE_DEVELOPMENT), gui_base->get_icon("Heart", "EditorIcons"));
...
void EditorNode::_scan_external_changes() {
	...
	String project_settings_path = ProjectSettings::get_singleton()->get_resource_path().plus_file("project.active");
	if (FileAccess::get_modified_time(project_settings_path) > ProjectSettings::get_singleton()->get_last_saved_time()) {
		TreeItem *ti = disk_changed_list->create_item(r);
		ti->set_text(0, "project.active");
		need_reload = true;
	}
...
void EditorNode::_menu_option_confirm(int p_option, bool p_confirmed) {
	...
	switch (p_option) {
		...
		case HELP_QA: {
			OS::get_singleton()->shell_open("https://activeengine.org/qa/");
		} break;
		case HELP_REPORT_A_BUG: {
			OS::get_singleton()->shell_open("https://github.com/activeengine/active/issues");
		} break;
		case HELP_SUGGEST_A_FEATURE: {
			OS::get_singleton()->shell_open("https://github.com/activeengine/active-proposals#readme");
		} break;
		case HELP_SEND_DOCS_FEEDBACK: {
			OS::get_singleton()->shell_open("https://github.com/activeengine/active-docs/issues");
		} break;
		case HELP_COMMUNITY: {
			OS::get_singleton()->shell_open("https://activeengine.org/community");
		} break;
		case HELP_ABOUT: {
			about->popup_centered_minsize(Size2(780, 500) * EDSCALE);
		} break;
		case HELP_SUPPORT_ACTIVE_DEVELOPMENT: {
			OS::get_singleton()->shell_open("https://activeengine.org/donate");
		} break;
...
bool EditorNode::call_build() {
	...
	for (int i = 0; i < build_callback_count && builds_successful; i++) {
		if (!build_callbacks[i]()) {
			ERR_PRINT("A Active Engine build callback failed.");
...
EditorNode::EditorNode() {
	...
	p->add_separator();
	p->add_icon_shortcut(gui_base->get_icon("Active", "EditorIcons"), ED_SHORTCUT("editor/about", TTR("About Active")), HELP_ABOUT);
	p->add_icon_shortcut(gui_base->get_icon("Heart", "EditorIcons"), ED_SHORTCUT("editor/support_development", TTR("Support Active Development")), HELP_SUPPORT_ACTIVE_DEVELOPMENT);
