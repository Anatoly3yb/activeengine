...
/*
 * This method is responsible for loading a project.active file and/or data file
...
Error ProjectSettings::_setup(const String &p_path, const String &p_main_pack, bool p_upwards, bool p_ignore_override) {
	...
	if (FileAccessNetworkClient::get_singleton()) {
		Error err = _load_settings_text_or_binary("res://project.active", "res://project.binary");
	...
	if (exec_path != "") {
		...
		// (This is the usual case when distributing a Active game.)
	...
	while (true) {
		err = _load_settings_text_or_binary(current_dir.plus_file("project.active"), current_dir.plus_file("project.binary"));
		if (err == OK && !p_ignore_override) {
			// Optional, we don't mind if it fails.
...
Error ProjectSettings::_load_settings_text(const String &p_path) {
	...
	while (true) {
		...
		if (err == ERR_FILE_EOF) {
			memdelete(f);
			// If we're loading a project.active from source code, we can operate some
			// ProjectSettings conversions if need be.
			_convert_to_last_version(config_version);
			last_save_time = FileAccess::get_modified_time(get_resource_path().plus_file("project.active"));
			return OK;
		} else if (err != OK) {
...
Error ProjectSettings::_load_settings_text_or_binary(const String &p_text_path, const String &p_bin_path) {
	// Attempt first to load the binary project.active file.
	...
	// Fallback to text-based project.active file if binary was not found.
...
Error ProjectSettings::save() {
	Error error = save_custom(get_resource_path().plus_file("project.active"));
	if (error == OK) {
		last_save_time = FileAccess::get_modified_time(get_resource_path().plus_file("project.active"));
	}
	return error;
...
Error ProjectSettings::_save_settings_text(const String &p_file, const Map<String, List<String>> &props, const CustomMap &p_custom, const String &p_custom_features) {
	...
	ERR_FAIL_COND_V_MSG(err != OK, err, "Couldn't save project.active - " + p_file + ".");
...
Error ProjectSettings::save_custom(const String &p_path, const CustomMap &p_custom, const Vector<String> &p_custom_features, bool p_merge_with_current) {
	...
	if (p_path.ends_with(".active") || p_path.ends_with("override.cfg")) {
...
ProjectSettings::ProjectSettings() {
	...
	// so that the values can be overridden from project.active or project.binary.
	...
	if (Engine::get_singleton()->has_singleton("ActiveSharp")) {
	...
	// Required to make the project setting appear even if the physics engine is ActivePhysics,
