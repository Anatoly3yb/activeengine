...
static uLong active_read(void *data, void *fdata, void *buf, uLong size) {
	FileAccess *f = (FileAccess *)fdata;
	f->get_buffer((uint8_t *)buf, size);
	return size;
}

static uLong active_write(voidpf opaque, voidpf stream, const void *buf, uLong size) {
	return 0;
}

static long active_tell(voidpf opaque, voidpf stream) {
	FileAccess *f = (FileAccess *)stream;
	return f->get_position();
}

static long active_seek(voidpf opaque, voidpf stream, uLong offset, int origin) {
	FileAccess *f = (FileAccess *)stream;
...
static int active_close(voidpf opaque, voidpf stream) {
	FileAccess *f = (FileAccess *)stream;
	if (f) {
		f->close();
		memdelete(f);
...
static int active_testerror(voidpf opaque, voidpf stream) {
	FileAccess *f = (FileAccess *)stream;
	return f->get_error() != OK ? 1 : 0;
}

static voidpf active_alloc(voidpf opaque, uInt items, uInt size) {
	return memalloc(items * size);
}

static void active_free(voidpf opaque, voidpf address) {
	memfree(address);
}
...
unzFile ZipArchive::get_file_handle(String p_file) const {
	...
	io.opaque = nullptr;
	io.zopen_file = active_open;
	io.zread_file = active_read;
	io.zwrite_file = active_write;

	io.ztell_file = active_tell;
	io.zseek_file = active_seek;
	io.zclose_file = active_close;
	io.zerror_file = active_testerror;

	io.alloc_mem = active_alloc;
	io.free_mem = active_free;
