...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the Active repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (active.tar.gz)"
    echo "  -v active version for file name (e.g. 4.0-stable)"
...
CURDIR=$(pwd)
TMPDIR=$(mktemp -d -t active-XXXXXX)
