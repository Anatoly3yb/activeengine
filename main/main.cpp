...
void initialize_physics() {
	...
	GLOBAL_DEF("physics/3d/active_physics/use_bvh", true);
	GLOBAL_DEF("physics/3d/active_physics/bvh_collision_margin", 0.1);
	ProjectSettings::get_singleton()->set_custom_property_info("physics/3d/active_physics/bvh_collision_margin", PropertyInfo(Variant::REAL, "physics/3d/active_physics/bvh_collision_margin", PROPERTY_HINT_RANGE, "0.0,2.0,0.01"));
...
void Main::print_help(const char *p_binary) {
	...
	OS::get_singleton()->print("(c) 2014-present Active Engine contributors.\n");
	OS::get_singleton()->print("(c) 2007-2014 Juan Linietsky, Ariel Manzur.\n");
	OS::get_singleton()->print("\n");
	OS::get_singleton()->print("Usage: %s [options] [path to scene or 'project.active' file]\n", p_binary);
	OS::get_singleton()->print("\n");
	...
	OS::get_singleton()->print("  --path <directory>               Path to a project (<directory> must contain a 'project.active' file).\n");
	OS::get_singleton()->print("  -u, --upwards                    Scan folders upwards for project.active file.\n");
	...
	OS::get_singleton()->print("  --gdnative-generate-json-api     Generate JSON dump of the Active API for GDNative bindings.\n");
#endif
...
Error Main::setup(const char *execpath, int argc, char *argv[], bool p_second_phase) {
	...
	GLOBAL_DEF("debug/settings/crash_handler/message.editor",
			String("Please include this when reporting the bug on: https://github.com/activeengine/active/issues"));
	...
	while (I) {
		...
		// Otherwise, Active would try to open a non-existent project on the first start and abort.
		...
		if (I->get() == "-h" || I->get() == "--help" || I->get() == "/?") { // display help
			...
			auto_quit = true;
		} else if (I->get().ends_with("project.active")) {
			String path;
			String file = I->get();
	...
	// 'project.active' file which will only be available through the network if this is enabled
	...
	GLOBAL_DEF("logging/file_logging/log_path", "user://logs/active.log");
	...
	if (tablet_driver == "") { // specified in project.active
	...
	if (audio_driver == "") { // specified in project.active
...
Error Main::setup2(Thread::ID p_main_tid_override) {
	...
	if (OS::get_singleton()->get_environment("USER") == "root" && !OS::get_singleton()->has_environment("ACTIVE_SILENCE_ROOT_WARNING")) {
		WARN_PRINT("Started the engine as `root`/superuser. This is a security risk, and subsystems like audio may not work correctly.\nSet the environment variable `ACTIVE_SILENCE_ROOT_WARNING` to 1 to silence this warning.");
	}
#endif
	...
	for (int i = 0; i < args.size(); i++) {
		...
		if (args[i] == "--check-only") {
			...
			if (args[i].ends_with(".scn") ||
				...
				// a file extension associated with Active scenes. This makes it possible
