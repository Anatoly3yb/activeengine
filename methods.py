...
# Get the "Active" folder name ahead of time
...
def get_version_info(module_version_string="", silent=False):
    ...
    if os.getenv("ACTIVE_VERSION_STATUS") != None:
        version_info["status"] = str(os.getenv("ACTIVE_VERSION_STATUS"))
        if not silent:
            print(
...
#define VERSION_DOCS_URL "https://docs.activeengine.org/en/" VERSION_DOCS_BRANCH
...
def detect_modules(search_path, recursive=False):
    ...
    def is_engine(path):
        ...
        # Active sources when using `custom_modules` build option.
        version_path = os.path.join(path, "version.py")
        if os.path.exists(version_path):
            with open(version_path) as f:
                if 'short_name = "active"' in f.read():
                    return True
        return False
...
def detect_visual_c_compiler_version(tools_env):
    ...
    # There are other architectures, but Active does not support them currently, so this function does not detect arm/amd64_arm
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        class ModuleConfigs(Mapping):
            ...
            def add_mode(
                ...
                self.arg_dict["runfile"] += [
                    f'bin\\active.windows.{config_id}.{plat_id}{f".{name}" if name else ""}.exe'
        ...
        env.MSVSProject(
            target=["#active" + env["MSVSPROJECTSUFFIX"]],
            incs=env.vs_incs,
            srcs=env.vs_srcs,
