...
Error MIDIDriverCoreMidi::open() {
	CFStringRef name = CFStringCreateWithCString(NULL, "Active", kCFStringEncodingASCII);
	...
	result = MIDIInputPortCreate(client, CFSTR("Active Input"), MIDIDriverCoreMidi::read, (void *)this, &port_in);
	if (result != noErr) {
